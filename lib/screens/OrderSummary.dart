...
import 'package:learning_mgt/screens/payment_success.dart';
                ...
                child: Consumer<OrderSummaryProvider>(
                  builder: (context, orderProvider, _) {
                    ...
                    return ElevatedButton(
                      onPressed: () async {
                        print('Course id for payment: ${widget.courseID}');
                        if (_selectedPaymentMethod == null) {
                          ShowDialogs.showToast('Please select a payment method');
                          return;
                        }

                        final candidateId = await SPManager().getUserId();
                        if (candidateId == null) {
                          ShowDialogs.showToast('User not authenticated');
                          return;
                        }
                        ...
                        orderProvider.savePayment(
                          ...
                          onSuccess: (message) {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => PaymentSuccessScreen(message: message),
                              ),
                            );
                          },
                          onError: (error) {
                            print('Payment error: $error');
                            ShowDialogs.showToast(error);
                          },
                        );
                      },
                      style: ElevatedButton.styleFrom(
