...
class _SeafarerSettingsState extends State<SeafarerSettings> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      //  if (!provider.isSeafarerLoading) {
      provider.getSeafarerListAPI();
  ...
  return Consumer<PersonalAccountProvider>(
      builder: (context, personalprovider, child) {
         if (personalprovider.isSeafarerLoading) {
      return Center(
                    ...
                    builder: (context, personalProvider, _) {
                      return ElevatedButton(
                        onPressed: personalProvider.isLoading
                            ? null
                            : () async {
                          await personalProvider.updateSeafarerDetails(context);
                          Navigator.pop(context);
                        },
                        style: ElevatedButton.styleFrom(
                          backgroundColor: personalProvider.isLoading
                              ? LearningColors.darkBlue.withOpacity(0.5)
